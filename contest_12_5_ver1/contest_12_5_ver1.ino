...
static unsigned long timePrev_exit = 0;
...
void loop()
{
  ...
  if (timeNow_G - timePrev_exit > 60000 && zoneNumber_G != 0) {
    zoneNumber_G = 0;
    mode_G = 0;
    timePrev_exit = timeNow_G;
  }
...
void sendData()
{
  ...
  if ( inByte == 0 || timeNow_G - timePrev > 500 || (Serial.available() > 0 && timeNow_G - timePrev > 50)) { // 50msごとにデータ送信
    ...
    send_time = (int)((timeNow_G - timePrev_exit) * 100);
